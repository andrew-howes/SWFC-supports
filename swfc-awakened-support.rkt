...
(require "swfc-awakened-cards-db.rkt")
...
(define (iterate-step-6)
  (refresh-requirements)
  (when (< (get-highest-requirement) 10.9)
...
(define (iterate-step-7)
  (refresh-requirements)
  (set! highest-req (get-highest-requirement))
  (when (< highest-req 5.5)
    (set! card-list-7 (get-candidate-list
                       remaining-requirements
                       card-list-6))
    
    (for ([card card-list-7])
      (when (>= (second card) highest-req)
        (set! card-7 card)
        (check-set)
        )
      )
